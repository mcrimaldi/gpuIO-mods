...
function main({ pane, contextID, glslVersion}) {
	...
	const rxnDiffusion = new GPUProgram(composer, {
		...
		void main() {
			...
			out_state = clamp(state + vec2(
				...
				u_diffusionB + ((u_diffusionB * laplacian.y * state.y) - u_gRate * state.y * (1.0 - (state.y/u_bMax)) - (state.y * u_dRate) - u_sensT * reaction)
			), 0.0, 1.0);
		}
	`,
		// fragmentShader: `
	...
	function loop() {
		...
		composer.step({
			program: PARAMS.renderLayer === 'Biomass B' ? renderA : renderB,
			input: state,
		});
	}
	...
	// TODO Aggiustare il tasto Reset
	function reset() {
	...
	// diffusion.addInput(PARAMS, 'diffusionA', { min: 1.0, max: 1.0, step: 0.1, label: 'Diffusion A' }).on('change', () => {
